...
const PayNow: FC = () => {
    ...
    if (tenantId) {
      try {
        ...
        if (response.ok) {
          ...
          const paymentStatusResponse = await fetch(`http://127.0.0.1:8000/api/tenant/${tenantId}/payment-status/`);
          if (paymentStatusResponse.ok) {
            const paymentStatus: PaymentStatus = await paymentStatusResponse.json();
 ...
 const generateMonths = (startDate: Date, monthlyDue: number, paymentStatus: PaymentStatus) => {
  ...
  for (let i = 1; i <= 12; i++) {
    const monthDate = new Date(startDate.getFullYear(), startDate.getMonth() + i, startDate.getDate());
    const monthStr = monthDate.toLocaleString('default', { month: 'long' });
    const monthKey = `${monthDate.getFullYear()}-${monthDate.getMonth() + 1}`;
    const isPaid = paymentStatus[monthDate.getFullYear().toString()]?.[monthStr] === 'paid';

    if (!monthsSet.has(monthKey) && monthDate >= startDate) { // Ensure no duplicate and only months from startDate onwards
  ...
  const startGenerationDate = new Date(lastPaidMonthDate.getFullYear(), lastPaidMonthDate.getMonth() + 1, lastPaidMonthDate.getDate());
  
  for (let i = 0; i < 12; i++) { // Start from the next month after last paid
    const monthDate = new Date(startGenerationDate.getFullYear(), startGenerationDate.getMonth() + i, startGenerationDate.getDate());
    const monthStr = monthDate.toLocaleString('default', { month: 'long' });
    const monthKey = `${monthDate.getFullYear()}-${monthDate.getMonth() + 1}`;
    const isPaid = paymentStatus[monthDate.getFullYear().toString()]?.[monthStr] === 'paid';
...
useEffect(() => {
  ...
  const totalPayable = unpaidMonths.reduce(
    (total, month) => {
      if (month.isChecked) {
...
return (
        ...
        <div className="flex flex-col gap-[47px] pb-[100px]">
          ...
          <div>
            <div className="relative mt-[-35px] flex flex-col">
              {months.filter(month => !month.isPaid).map((month, index) => (
                <PayNowButtonRowOne
                  key={index}
                  month={month.month}
    ...
    {showCashPaymentModal && tenantId && (
      <CashPaymentModal
        ...
        onPaymentSuccess={(paidMonthKeys) => handlePaymentSuccess(paidMonthKeys)}
      />
    )}
    </>
